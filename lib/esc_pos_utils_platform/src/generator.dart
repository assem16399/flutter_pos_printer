...
class Generator {
  ...
  List<int> _toRasterFormat(img.Image imgSrc) {
    ...
    print('Buffer of image is: ${buffer.length}');

    for (int i = 0; i < buffer.length; i += image.data!.numChannels) {
    ...
    if (widthPx % 8 != 0) {
      ...
      print('One Channel Bytes length: ${oneChannelBytes.length}');
      print('Image Width: ${widthPx}');
      print('Image Height: ${heightPx}');
      print("extra length: ${extra.length}");
      print('Target Width: $targetWidth');
      print('Missing PX: $missingPx');
      for (int i = 0; i < heightPx; i++) {
        final pos = (i * widthPx + widthPx) + i * missingPx;
        print('i: $i');
        print('Position on insertion: $pos');
        oneChannelBytes.insertAll(pos, extra);
        print(
            'One Channel Bytes After insertion length: ${oneChannelBytes.length}');
      }
    }
  ...
  List<int> imageRaster(
    ...
    final List<int> rasterizedData = image.buffer.asUint8List().toList();
